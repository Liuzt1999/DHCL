...
def evaluate_hamming2(model, dataloader, hash_K):
    print('eveluate hamming2')
    nb_classes = dataloader.dataset.nb_classes()
    ...
    for i in range(len(X)):
        for j in range(len(X)):
            cos_sim[i][j]=hash_K-hamming_distance(X[i],X[j]);
    Y = T[cos_sim.topk(1 + K)[1][:, 1:]]
    Y = Y.float().cpu()

    aps = []
    for kk in [1, 10, 20, 40, 60]:
        map_at_k = calc_map_at_k0(T, Y, kk)
        aps.append(map_at_k)
        print("map@{} : {:.3f}".format(kk, 100 * map_at_k))

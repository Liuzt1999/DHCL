...
class bn_inception(nn.Module):
    def __init__(self, embedding_size, pretrained = True, is_norm=True, bn_freeze = True, classes = 21):
...
#           weight = model_zoo.load_url('http://data.lip6.fr/cadene/pretrainedmodels/bn_inception-239d2248.pth')
        ...
        self.model.embed = nn.tanh(self.model.embedding)
        self.model.embedding_label = nn.Linear(self.model.embed, self.classes)
        self.model.embed_label = nn.softmax(self.model.embedding_label)
...
class BNInception(nn.Module):
    ...
    def forward(self, input):
        x = self.features(input)   #output==[150,1024,7,7]
        avg_x = self.gap(x)        #[150,1024,1,1]
        max_x = self.gmp(x)        #[150,1024,1,1]
    
        x = avg_x + max_x
        x = x.view(x.size(0), -1)  #[150,1024]
        x = self.embedding(x)      #[150,512]
        y = self.embedding_label(x) #理论上应该是分类标签的长度
        # print('++++')
        # print(y.size)
        if self.is_norm:
            x = self.l2_norm(x)

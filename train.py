...
import losses
import utils
from dataset.UCMD import UCMD
from dataset.AID import AID
...
parser.add_argument('--dataset', 
    default='AID',
    help = 'Training dataset, e.g. cub, cars, SOP, Inshop, UCMD, AID'
)
# 这个地方是最后输出层的维度，后面看一下是在这里改哈希维度，还是下面那个
parser.add_argument('--embedding-size', default = 32, type = int,
    dest = 'sz_embedding',
    help = 'Size of embedding that is appended to backbone model.'
)
# 调节哈希吗长度的地方，就在这做不同对比实验的调整
...
parser.add_argument('--classes', default = 21, type = int,
    dest = 'classes',
    help = 'Number of classes per dataset.'
)
...
parser.add_argument('--epochs', default = 10, type = int,  #default=60
    dest = 'nb_epochs',
...
parser.add_argument('--loss', default = 'Proxy_Anchor_with_label',
...
nb_classes = trn_dataset.nb_classes()  # len(classes)?
...
if args.model.find('googlenet')+1:
    ...
    model = bn_inception(embedding_size=args.sz_embedding, pretrained=True, is_norm=args.l2_norm, bn_freeze = args.bn_freeze,classes = args.classes)
...
elif args.loss == 'Proxy_Anchor_with_label':
    criterion = losses.Proxy_Anchor_with_label(nb_classes = nb_classes, sz_embed = args.sz_embedding, mrg = args.mrg, alpha = args.alpha).cuda()
...
if args.loss == 'Proxy_Anchor' or args.loss == 'Proxy_Anchor_circle' or args.loss == 'Proxy_Anchor_with_Label':
...
for epoch in range(0, args.nb_epochs):
    ...
    #x(150*3*224*224)  y(150),  m(150,512)
    ...
    if(epoch >= 0):
        with torch.no_grad():
            print("**Evaluating...**")
            if args.dataset == 'UCMD' or args.dataset == 'AID':
                Recalls = utils.evaluate_hamming2(model, dl_ev, args.sz_embedding)
